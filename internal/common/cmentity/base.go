...
type Entity struct {
	...
	CreatedAt TimeAt `json:"-" bson:"created_at"`
	UpdatedAt TimeAt `json:"-" bson:"updated_at"`
...
func NewIDFromHexString(hexStr string) (*ID, error) {
	...
	return util.ToPtr(ID(objectID.Hex())), nil
}
...
// TimeAt is a custom type that helps to Marshal a Datetime value from Database.
...
func (c TimeAt) MarshalJSON() ([]byte, error) {
	ts := c.Time().Unix()
	stamp := fmt.Sprint(ts)
...
func (c *TimeAt) UnmarshalJSON(b []byte) error {
...
func (c TimeAt) GetBSON() (any, error) {
	if c.Time().IsZero() {
		return time.Now(), nil
	}
...
func (c *TimeAt) SetBSON(raw bson.RawValue) error {
